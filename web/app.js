...
const state = {
    ...
    // State transitorio de la vista (lo que toca dentro del frame). El
    // resto de la configuración persiste en localStorage vía `HoyV2Mix`.
    hoyV2FeedIndex: 0,          // index del clip activo en feed vertical
    hoyV2Prediction: null,      // opción del predictor seleccionada

    // Selección curada de equipos + jugadores favoritos.
...
// ════════════════════════════════════════════════════════════════
// HOY V2 — "Mi Mix" (flag `fan.hoy.concept-mix`)
// Rediseño elegido del "Hoy": una Home modular y configurable. Sus
// módulos se activan por separado vía los sub-flags de `concept-mix`.
// ════════════════════════════════════════════════════════════════
...
// ── Top-level renderer ──────────────────────────────────────────
// El Hoy v2 se queda con un único concepto: "Mi Mix" (flag
// `fan.hoy.concept-mix`). Sus módulos viven en sub-flags toggleables.
function renderHoyV2Mix() {
    const hifi = Flags.isEnabled('fan.hoy.concept-mix.hi-fi');
    return `
        <div class="hv2-wrap" data-hv2-concept="MIX" data-hv2-matchday="0">
            <!-- El sello del concepto vive fuera del frame, en el panel
                 resumen lateral (#hv2RecapSlot). El frame se queda limpio
                 para imitar mejor cómo se vería la app real. -->
            <div class="hv2-scroll ${hifi ? 'is-hifi' : ''}" id="hv2Scroll" data-hv2-concept-bg="MIX">
                ${renderHV2ConceptMix()}
                <div style="height: 28px"></div>
            </div>
        </div>
...
// Tagline corto del concepto — usado en el panel resumen lateral
// (#hv2RecapSlot) en desktop. Mantiene la identidad fuera del frame.
const HV2_TAGLINE = {
    MIX: 'Mezcla curada: tú eliges qué módulos viven en tu Hoy.'
};

// Render compacto de noticias para Mi Mix: 4 noticias con thumb
...
// la app (Fan y VIP), no sólo para Hoy V2. CSS lo oculta por debajo de
// 1100px. Estructura:
//   1. Cabecera: contexto (App · Tab actual).
//   2. Cuando estás en Fan App > Hoy con `fan.hoy.concept-mix` ON,
//      tarjeta destacada de Mi Mix (con build/lift/riesgo).
...
function renderHV2Recap() {
    ...
    // ── Tarjeta del concepto (sólo en Fan > Hoy con Mi Mix) ──────
    let conceptCard = '';
    const inHoyV2 = state.app === 'fan' && state.tab === 'hoy' && Flags.isEnabled('fan.hoy.concept-mix');
    if (inHoyV2) {
        const meta = HV2_CONCEPTS[0];
        conceptCard = `
            <div class="hv2-recap-concept hv2-recap-MIX">
                ...
                <p class="hv2-recap-line">${HV2_TAGLINE.MIX}</p>
...
// favoritos. Cada bloque es toggleable individualmente como sub-flag
// bajo `fan.hoy.concept-mix`.
// ════════════════════════════════════════════════════════════════
...
const HV2_MIX_BLOCKS = {
    ...
    'fan.hoy.concept-mix.upcoming':     renderHV2MixUpcoming,
...
// ── Event wiring (Hoy V2 — Mi Mix) ──────────────────────────────
function attachHoyV2OptionsListeners() {
    if (!Flags.isEnabled('fan.hoy.concept-mix') || state.tab !== 'hoy' || state.app !== 'fan') return;

    // Feed vertical (↑/↓) y predictor — bloques reutilizados por Mi Mix.
...
// ── Responsive drawers ──────────────────────────────────────────
// En viewports estrechos los paneles laterales pasan a cajones
// off-canvas (ver CSS). Aquí wireamos los botones flotantes, el scrim,
// Escape y el cierre al navegar.
function setupResponsiveDrawers() {
    const left  = document.getElementById('sidePanel');
    const right = document.getElementById('sidePanelRight');
    const scrim = document.getElementById('drawerScrim');
    const tLeft  = document.getElementById('drawerToggleLeft');
    const tRight = document.getElementById('drawerToggleRight');
    if (!scrim) return;

    let scrimHideTimer = null;
    const anyOpen = () =>
        (left && left.classList.contains('drawer-open')) ||
        (right && right.classList.contains('drawer-open'));

    const showScrim = () => {
        clearTimeout(scrimHideTimer);
        scrim.hidden = false;
        void scrim.offsetWidth;      // reflow para que transicione el opacity
        scrim.classList.add('show');
    };
    const hideScrimIfNone = () => {
        if (anyOpen()) return;
        scrim.classList.remove('show');
        scrimHideTimer = setTimeout(() => { scrim.hidden = true; }, 260);
    };

    const openDrawer = (panel, toggle) => {
        if (!panel) return;
        panel.classList.add('drawer-open');
        if (toggle) {
            toggle.setAttribute('aria-expanded', 'true');
            // Ocultamos el botón mientras el cajón está abierto para que no
            // se solape con el contenido del panel; se cierra con el scrim,
            // Escape o navegando. (Revierte al display de la media query.)
            toggle.style.display = 'none';
        }
        showScrim();
    };
    const closeDrawer = (panel, toggle) => {
        if (!panel) return;
    ...
    const closeAll = () => { closeDrawer(left, tLeft); closeDrawer(right, tRight); };

    if (tLeft) tLeft.addEventListener('click', () =>
        (left.classList.contains('drawer-open') ? closeDrawer(left, tLeft) : openDrawer(left, tLeft)));
    if (tRight) tRight.addEventListener('click', () =>
        (right.classList.contains('drawer-open') ? closeDrawer(right, tRight) : openDrawer(right, tRight)));

    scrim.addEventListener('click', closeAll);
    document.addEventListener('keydown', (e) => { if (e.key === 'Escape' && anyOpen()) closeAll(); });

    // Navegar por el menú cierra el cajón izquierdo (delegación: #sideNav
    // se re-renderiza, pero el listener vive en el contenedor estable).
    const sideNav = document.getElementById('sideNav');
    if (sideNav) sideNav.addEventListener('click', (e) => {
        if (e.target.closest('.nav-tab, .nav-sub') && left && left.classList.contains('drawer-open')) {
            closeDrawer(left, tLeft);
        }
    });

    // Al ensanchar por encima del breakpoint, resetea para no dejar
    // clases de cajón colgadas cuando los paneles vuelven al grid.
    window.addEventListener('resize', () => { if (window.innerWidth > 1280) closeAll(); });
}
...
// ================================================================
// AUTH UI — extraido a auth_ui.js (ver ese fichero). Se carga como
// script aparte en index.html; los simbolos siguen siendo globales.
// ================================================================
...
function flagRowHTML(f, { indent = false, hasKids = false, collapsed = false, kidsTotal = 0, kidsActive = 0 } = {}) {
    ...
    // Sólo los hijos (con `requires`) son arrastrables. Mover un hijo
    // dentro de su padre cambia el orden de renderizado de los bloques
    // del concepto en el frame.
    ...
    return `
        <div class="flag-row ${indent ? 'is-child' : ''} ${locked ? 'is-locked' : ''} ${draggable ? 'is-draggable' : ''} ${hasKids ? 'has-kids' : ''} ${collapsed ? 'is-collapsed' : ''}"
            ...
            ${draggable ? `
                <span class="flag-row-handle" aria-label="Arrastrar para reordenar" title="Arrastra para reordenar dentro de su funcionalidad padre. El orden se refleja en el frame.">
    ...
    host.innerHTML = `
        <div class="flags-panel ${isOpen ? 'open' : 'closed'}" id="flagsPanel">
            ...
            <div class="flags-body" ${isOpen ? '' : 'hidden'}>
                ...
                <!-- Compartir: codifica la configuración actual en un enlace -->
                <button class="flags-share-btn" id="flagsShareLink" title="Copia un enlace que reproduce exactamente esta configuración de funcionalidades">
                    <svg viewBox="0 0 24 24" width="13" height="13" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round">
                        <path d="M9.5 13.5a3 3 0 0 0 4.3.4l2.7-2.7a3 3 0 0 0-4.2-4.2l-1.1 1"/>
                        <path d="M14.5 10.5a3 3 0 0 0-4.3-.4L7.5 12.8a3 3 0 0 0 4.2 4.2l1.1-1"/>
                    </svg>
                    <span>Copiar enlace de esta configuración</span>
                </button>
                ${(window.JiraInitiatives && JiraInitiatives.count && JiraInitiatives.count() > 0) ? `
                <!-- Ver todas las iniciativas JIRA agrupadas por epic -->
                <button class="flags-share-btn flags-jira-btn" id="flagsJiraList" title="Ver todas las iniciativas JIRA agrupadas por epic, con filtro">
                    <svg viewBox="0 0 24 24" width="13" height="13" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round">
                        <rect x="3.5" y="3.5" width="17" height="17" rx="2.5"/><path d="M8 8.5h8M8 12h8M8 15.5h5"/>
                    </svg>
                    <span>Ver iniciativas JIRA (${JiraInitiatives.count()})</span>
                </button>` : ''}
            </div>
        </div>
    `;
...
function _attachFlagsPanelListeners() {
    ...
    // Drag & drop para reordenar sub-flags dentro del mismo padre.
    // Sólo los hijos (con data-flag-parent) son arrastrables. Al
    // soltar uno sobre otro hermano, recolocamos el dragKey
    // ANTES del droppedOn y persistimos vía Flags.setOrder. El
    // re-render (vía Flags.onChange) refleja el nuevo orden en
    // el frame del móvil.
    let dragKey = null;
    let dragParent = null;
    $$('.flag-row[data-flag-parent]').forEach(row => {
        row.addEventListener('dragstart', e => {
            dragKey = row.dataset.flagRow;
            dragParent = row.dataset.flagParent;
        ...
        row.addEventListener('dragend', () => {
            ...
            dragKey = null;
            dragParent = null;
            $$('.flag-row.is-drop-target').forEach(r => r.classList.remove('is-drop-target'));
        });
        row.addEventListener('dragover', e => {
            if (!dragKey || row.dataset.flagParent !== dragParent) return;
            if (row.dataset.flagRow === dragKey) return;
            e.preventDefault();
        ...
        row.addEventListener('drop', e => {
            if (!dragKey || row.dataset.flagParent !== dragParent) return;
            e.preventDefault();
            const droppedOn = row.dataset.flagRow;
            row.classList.remove('is-drop-target');
            if (droppedOn === dragKey) return;
            const current = Flags.getOrderedChildKeys(dragParent);
    ...
    // Copiar enlace — codifica la configuración actual (diff vs defaults
    // + orden custom) en ?flags=… para reproducirla en otro navegador.
    const shareBtn = $('#flagsShareLink');
    if (shareBtn) shareBtn.addEventListener('click', async () => {
        const encoded = Flags.encodeShareState();
        const params = new URLSearchParams(location.search);
        if (encoded) params.set('flags', encoded); else params.delete('flags');
        const qs = params.toString();
        const url = location.origin + location.pathname + (qs ? '?' + qs : '') + location.hash;
        const ok = window.MockUI && await MockUI.copyToClipboard(url);
        if (ok) {
            toast(encoded
                ? 'Enlace copiado con tu configuración actual.'
                : 'Enlace copiado (todo por defecto).', { type: 'success' });
        } else {
            toast('No se pudo copiar el enlace.', { type: 'error' });
        }
    });

    // Ver todas las iniciativas JIRA (listado agrupado por epic).
    const jiraListBtn = $('#flagsJiraList');
    if (jiraListBtn) jiraListBtn.addEventListener('click', () => {
        if (window.JiraInitiatives && JiraInitiatives.showList) JiraInitiatives.showList();
    });
}
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    try {
        ...
        // Apply a shared flag configuration from the URL (?flags=…) so a
        // link reproduces an exact setup, then strip the param (the state
        // now lives in localStorage). Runs before the first render.
        try {
            const params = new URLSearchParams(location.search);
            const shared = params.get('flags');
            if (shared && typeof Flags !== 'undefined' && Flags.applyShareState(shared)) {
                params.delete('flags');
                const qs = params.toString();
                history.replaceState(null, '', location.pathname + (qs ? '?' + qs : '') + location.hash);
                if (window.MockUI) MockUI.toast('Configuración cargada desde el enlace.', { type: 'info' });
            }
        } catch {}
        ...
        // Cajones off-canvas para viewports estrechos.
